    ...
    <div class="min-h-screen bg-gray-50 text-gray-800">
        <div class="px-6 py-6">
            ...
            <div
              ...
              class="absolute right-0 top-12 w-80 bg-white rounded-lg shadow-lg border z-50"
            >
                            ...
                            <div class="p-6" v-if="selectedNotification">
                                <div class="flex items-start space-x-3 mb-4">
                                    <svg v-if="selectedNotification.type === 'leave_approved'"
                                        class="w-8 h-8 text-green-500 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <svg v-else-if="selectedNotification.type === 'leave_rejected'"
                                        class="w-8 h-8 text-red-500 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                ...
                                <div v-if="selectedNotification.data" class="bg-gray-50 rounded-lg p-4 mb-4">
                                    ...
                                    <div class="text-sm text-gray-600">
                                        ...
                                        <p
                                            v-if="selectedNotification.data.from_date && selectedNotification.data.to_date">
            ...
            <div class="space-y-3">
                <div class="bg-gray-50 rounded-lg p-4">
                    <div class="grid grid-cols-1 gap-3 text-sm">
                        ...
                        <!-- Approver/Rejector -->
                        <div v-if="parseNotificationDetails(notification.message).actionBy" 
                             class="flex items-center space-x-2">
                        ...
                        <!-- Rejection Reason -->
                        <div v-if="notification.type === 'leave_rejected' && parseNotificationDetails(notification.message).reason" 
                             class="mt-3 ">
...
const filteredNotifications = computed(() => {
    if (!statusFilter.value || statusFilter.value === 'all') return notifications.value; // Return all if no filter or 'all' is selected
    return notifications.value.filter(notification => {
        if (statusFilter.value === 'pending') return notification.type === 'leave_pending'; // Ensure pending requests are filtered correctly
...
const setStatusFilter = (status) => {
    ...
    currentStatus.value = status === 'all' ? 'All Requests' : status.charAt(0).toUpperCase() + status.slice(1); // Capitalize the first letter or set to 'All Requests'
};
