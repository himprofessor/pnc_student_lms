<template>
  <div class="min-h-screen bg-gray-100 text-gray-800 font-sans">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 px-6 mb-6">
      <!-- Pending -->
      <div
        class="bg-white p-5 rounded-lg shadow-sm flex items-center space-x-4"
      >
    ...
    <div class="mt-6 px-6">
      <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <h3 class="text-lg font-semibold px-6 pt-4 text-gray-700">
          Leave Requests
        </h3>
        <p class="text-gray-600 px-6 pb-4">
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div
        class="bg-gradient-to-r from-pink-600 to-blue-500 px-3 py-2 text-white relative"
      >
        <button
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p class="text-gray-900 font-semibold">
                    {{ formatDate(detail.from_date) }}
                  </p>
                </div>
                <div>
                  <p class="text-sm text-gray-500 font-medium">To Date</p>
                  <p class="text-gray-900 font-semibold">
                    {{ formatDate(detail.to_date) }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-2">
              ...
              <div class="space-y-2">
                ...
                <button
                  ...
                  @click="openRejectDialog(detail.id)"
                >
                  Reject
                </button>
              </div>
          ...
          <div class="space-y-6">
            <div class="bg-gray-50 rounded-xl p-6">
              ...
              <div v-if="detail.supporting_documents" class="space-y-4">
                ...
                >
                  <div
                    v-if="isImageFile(detail.supporting_documents)"
                    class="text-center"
                  >
                    <img
                  ...
                  <div
                    v-else-if="isPdfFile(detail.supporting_documents)"
                    class="text-center"
                  >
                    <iframe
                      :src="getDocumentUrl(detail.supporting_documents)"
                  ...
                  <div v-else class="text-center py-8">
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.767.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
                      />
                    </svg>
                    ...
                    <p class="text-sm text-gray-500">
                      {{ getFileName(detail.supporting_documents) }}
                    </p>
                  </div>
                </div>
...
const fetchLeaveRequests = async () => {
  try {
    const res = await axios.get(
      "http://127.0.0.1:8000/api/educator/leave-requests",
      {
        headers: {
    ...
    if (res.data && res.data.length > 0) {
      ...
      console.log(
        "Available fields in first request:",
      ...
      console.log(
        ...
        firstRequest.student?.profile_image
      );
...
const viewDetail = async (id) => {
  try {
    const res = await axios.get(
      `http://127.0.0.1:8000/api/educator/leave-request/${id}`,
      {
        headers: {
    ...
    detail.value = {
      ...request,
      profile_image:
        request.profile_image || request.student_img || request.img || null,
...
const approve = async (id) => {
  await axios.post(
    `http://127.0.0.1:8000/api/educator/leave-request/${id}/approve`,
    {},
    {
...
const handleRejectSubmit = async (reason) => {
  try {
    ...
    await axios.post(
      `http://127.0.0.1:8000/api/educator/leave-request/${rejectLeaveId.value}/reject`,
    ...
    // Optional: Refresh the detail object from backend
    const response = await axios.get(
      `http://127.0.0.1:8000/api/educator/leave-request/${rejectLeaveId.value}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    detail.value = response.data.data; // update UI

    showRejectDialog.value = false;
  } catch (error) {
    console.error("Failed to reject leave:", error);
    alert("Error: Could not reject leave request.");
  }
};
...
const approvedTodayCount = computed(() => {
  ...
  return leaveRequests.value.filter(
    (r) => r.status === "Approved" && r.submitted.startsWith(today)
  ).length;
});
...
const getDetailImageUrl = () => {
  if (!detail.value || !detail.value.profile_image || detailImageError.value) {
    ...
    const initial = detail.value?.student
      ? detail.value.student.charAt(0).toUpperCase()
      : "?";
  ...
  console.log(
    "Using detail student profile image:",
    detail.value.profile_image
  );
...
const getProfileImageUrl = (request) => {
  ...
  if (!imageField) {
    const initial = request.student
      ? request.student.charAt(0).toUpperCase()
      : "?";
    console.log(
      `❌ No img field found for student ${request.student_id}, returning placeholder: ${initial}`
    );
    return `data:image/svg+xml;base64,`;
  }
  ...
  console.log(
    `✅ Final image URL for student ${request.student_id}:`,
    finalUrl
  );
  return finalUrl;
};
...
const handleImageError = (event, request) => {
  console.log(
    `Profile image failed to load for student ${request.student}, falling back to initials`
  );
  const initial = request.student
    ? request.student.charAt(0).toUpperCase()
    : "?";
